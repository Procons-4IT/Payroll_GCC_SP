Public Class clsSalaryIncrement
    ...
    End Sub
    Public Sub LoadForm(ByVal aEmpID As String, ByVal aTAempID As String)
        If oApplication.Utilities.validateAuthorization(oApplication.Company.UserSignature, frm_SalaryIncrement) = False Then
            oApplication.Utilities.Message("You are not authorized to do this action", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
            Exit Sub
        End If
        oForm = oApplication.Utilities.LoadForm(xml_SalaryIncrement, frm_SalaryIncrement)
...
#Region "Databind"
    Private Sub Databind(ByVal aform As SAPbouiCOM.Form)
        Try
            ...
            dtTemp.ExecuteQuery("Select *,Code 'Ref' from [@Z_PAY11] where U_Z_Empid='" & oApplication.Utilities.getEdittextvalue(aform, "4") & "'  order by U_Z_StartDate")
...
#Region "FormatGrid"
    Private Sub Formatgrid(ByVal agrid As SAPbouiCOM.Grid)
        ...
        agrid.Columns.Item(2).TitleObject.Caption = "EmpID"
        agrid.Columns.Item(0).Visible = False
        agrid.Columns.Item(1).Visible = False
        agrid.Columns.Item(2).Visible = False
        agrid.Columns.Item(3).TitleObject.Caption = "Effective From"
        agrid.Columns.Item(4).TitleObject.Caption = "Effective To"
        agrid.Columns.Item(4).Editable = False
        agrid.Columns.Item(5).TitleObject.Caption = "Increment Amount"
        agrid.Columns.Item(6).TitleObject.Caption = "Consolidated Increment"
...
#Region "CommitTrans"
    Private Sub Committrans(ByVal strChoice As String)
        ...
        If strChoice = "Cancel" Then
            oTemprec.DoQuery("Update [@Z_PAY11] set NAME=CODE where Name Like '%_XD'")
        Else
            ...
            '    oTemprec.MoveNext()
            'Next
            oTemprec.DoQuery("Delete from  [@Z_PAY11]  where NAME Like '%_XD'")
        End If
...
#Region "AddtoUDT"
    Private Sub UpdatePayrollIncrementEndDate(aEmpID As String)
        ...
        oRec.DoQuery("Select * from ""@Z_PAY11"" where ""U_Z_EmpID""='" & aEmpID & "' order by ""U_Z_StartDate"" Desc ")
        Dim dtFromdate, dtEndDate As Date

        For intRow As Integer = 0 To oRec.RecordCount - 1
            If intRow > 0 Then
                oRec1.DoQuery("Update ""@Z_PAY11"" set ""U_Z_EndDate""='" & dtEndDate.ToString("yyyy-MM-dd") & "' where ""Code""='" & oRec.Fields.Item("Code").Value & "'")
            End If
    ...
    Private Function AddtoUDT1(ByVal aform As SAPbouiCOM.Form) As Boolean
        ...
        For intRow As Integer = 0 To oGrid.DataTable.Rows.Count - 1
            ...
            If strESocial <> "" Then
                ...
                If oUserTable.GetByKey(strCode) = False Then
                    ...
                    oUserTable.UserFields.Fields.Item("U_Z_InrAmt").Value = oGrid.DataTable.GetValue(6, intRow)
        ...
        Committrans("Add")
        UpdatePayrollIncrementEndDate(strEmp)
        Databind(aform)
        Return True
    End Function
#End Region
    ...
    Private Sub RemoveRow(ByVal intRow As Integer, ByVal agrid As SAPbouiCOM.Grid)
        ...
        For intRow = 0 To agrid.DataTable.Rows.Count - 1
            If agrid.Rows.IsSelected(intRow) Then
                ...
                otemprec = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                oApplication.Utilities.ExecuteSQL(oTemp, "update [@Z_PAY11] set  NAME =NAME +'_XD'  where CODE='" & strCode & "'")
...
#Region "Validate Grid details"
    Private Function validation(ByVal aGrid As SAPbouiCOM.Grid) As Boolean
        ...
        For intInnerLoop As Integer = 0 To aGrid.DataTable.Rows.Count - 1
            ...
            dtfromdate = oGrid.DataTable.GetValue(3, intInnerLoop)
            dttodate = (oGrid.DataTable.GetValue(4, intInnerLoop))
            If strEname1 <> "" Then
                If aGrid.DataTable.GetValue(3, intInnerLoop) > aGrid.DataTable.GetValue(4, intInnerLoop) Then
            ...
            If intInnerLoop > 0 Then
                If aGrid.DataTable.GetValue(3, intInnerLoop) <= aGrid.DataTable.GetValue(3, intInnerLoop - 1) Then
...
#Region "Item Event"
    Public Overrides Sub ItemEvent(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean)
        Try
            If pVal.FormTypeEx = frm_SalaryIncrement Then
