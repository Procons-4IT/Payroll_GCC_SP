Public Class clsPayrollTransaction_Offcycle
    Inherits clsBase
    ...
    Private Sub LoadForm()
        If oApplication.Utilities.validateAuthorization(oApplication.Company.UserSignature, frm_OffToolTransaction) = False Then
            oApplication.Utilities.Message("You are not authorized to do this action", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
            Exit Sub
        End If
        oForm = oApplication.Utilities.LoadForm(xml_OffToolTransaction, frm_OffToolTransaction)
        ...
        Try
            ...
            Select Case aChoice
                ...
                Case "Trans"
                    ...
                    oGrid.Columns.Item("U_Z_AffDedu").Type = SAPbouiCOM.BoGridColumnType.gct_CheckBox
                    oGrid.Columns.Item("U_Z_AffDedu").TitleObject.Caption = "Part of Payroll Deduction"
                    oGrid.Columns.Item("U_Z_AffDedu").Editable = False

                    oGrid.Columns.Item("U_Z_DedMonth").TitleObject.Caption = "Deduction Month"
                    oGrid.Columns.Item("U_Z_DedMonth").Type = SAPbouiCOM.BoGridColumnType.gct_ComboBox
                    oComboColumn = oGrid.Columns.Item("U_Z_DedMonth")
                    oComboColumn.ValidValues.Add("0", "")
                    For intRow As Integer = 1 To 12
                        oComboColumn.ValidValues.Add(intRow, MonthName(intRow))
                    Next
                    ...
                    oGrid.Columns.Item("U_Z_DedYear").Type = SAPbouiCOM.BoGridColumnType.gct_ComboBox
                    oGrid.Columns.Item("U_Z_DedYear").TitleObject.Caption = "Deduction Year"
                    oGrid.Columns.Item("U_Z_DedYear").Visible = True

                    oComboColumn = oGrid.Columns.Item("U_Z_DedYear")
                    oComboColumn.ValidValues.Add("0", "")
                    For intRow As Integer = 2010 To 2050
                        oComboColumn.ValidValues.Add(intRow, intRow)
                    Next
        ...
        For intRow As Integer = 0 To oGrid.DataTable.Rows.Count - 1
            ...
            If strType <> "" And oGrid.DataTable.GetValue("U_Z_EMPID", intRow) <> "" Then
                ...
                If oGrid.DataTable.GetValue("Code", intRow) = "" Then
                    ...
                    oUserTable.UserFields.Fields.Item("U_Z_DedMonth").Value = oGrid.DataTable.GetValue("U_Z_DedMonth", intRow)
                    oUserTable.UserFields.Fields.Item("U_Z_DedYear").Value = oGrid.DataTable.GetValue("U_Z_DedYear", intRow)
                    ...
                    End If
                    oUserTable.UserFields.Fields.Item("U_Z_offTool").Value = "Y"
...
#Region "Populate Employee Details"
    Private Sub PopulateEmployeeDetails(ByVal aForm As SAPbouiCOM.Form)
        Try
            ...
            strQuery = "SELECT T0.[U_Z_EmpId],T0.[empID], T0.[firstName] + isnull( T0.[middleName],'') + isnull(T0.[lastName],'') 'Name', T1.[U_Z_Type], T1.[U_Z_TrnsCode], convert(varchar,T1.U_Z_Month) 'U_Z_Month',convert(varchar,T1.U_Z_Year) 'U_Z_Year', T1.[U_Z_StartDate], T1.[U_Z_EndDate], T1.[U_Z_Amount], T1.[U_Z_NoofHours], T1.[U_Z_Notes],T1.U_Z_JVNo 'Journal Ref' FROM OHEM T0 left outer Join  [dbo].[@Z_PAY_TRANS]  T1 on T1.U_Z_EMPID=T0.empID"
            strQuery = strQuery & " where  isnull(T1.U_Z_OffTool,'N')='Y' and " & strEmpCondition & " and " & strDept & " and " & strPosition & " and " & strBranch & " and  U_Z_Year=" & CInt(strYear) & " and U_Z_Month=" & CInt(strMonth) & " and T0.""U_Z_CompNo""='" & strCompany & "'  order by T0.empID"
            ...
            If otest.RecordCount > 0 Then
                If otest.Fields.Item("U_Z_Process").Value = "Y" Then
                    aForm.Items.Item("4").Enabled = True ' False
    ...
    Private Sub TransactionDetails(ByVal aform As SAPbouiCOM.Form)
        Try
            ...
            strQuery = "SELECT T0.[U_Z_EmpId1], T0.[Code], T0.[Name], T0.[U_Z_EMPID],T0.""U_Z_EMPNAME"", T0.[U_Z_Type], T0.[U_Z_TrnsCode], Convert(Varchar,T0.[U_Z_Month]) 'U_Z_Month', Convert(varchar,T0.[U_Z_Year]) 'U_Z_Year', T0.[U_Z_StartDate], T0.[U_Z_EndDate], isnull(U_Z_AffDedu,'N') 'U_Z_AffDedu' , Convert(Varchar,T0.[U_Z_DedMonth]) 'U_Z_DedMonth', Convert(varchar,T0.[U_Z_DedYear]) 'U_Z_DedYear', T0.[U_Z_Amount], T0.[U_Z_NoofHours], T0.[U_Z_Notes] ,T0.U_Z_Posted  FROM [dbo].[@Z_PAY_TRANS]  T0 inner Join OHEM T1 on T1.empID=T0.U_Z_EMPID"
            strQuery = strQuery & " where isnull(T0.U_Z_OffTool,'N')='Y' and isnull(T0.U_Z_Posted,'N')='N' and " & strEmpCondition & " and U_Z_MOnth=" & CInt(strmonth) & " and U_Z_Year=" & CInt(stryear) & " and T1.""U_Z_CompNo""='" & strCompany & "' "
            ...
            If agrid.Rows.IsSelected(intRow) Then
                ...
                If strCode <> "" Then
                    If oGrid.DataTable.GetValue("U_Z_Posted", intRow) = "Y" Then
                        oApplication.Utilities.Message("Transaction already posted. you can not delete transaction", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                        Exit Sub
                    End If
                    otemprec = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
...
#End Region
    Private Function getDailyrate(ByVal aCode As String, ByVal aLeaveType As String, ByVal aBasic As Double, Optional ByVal LeaveCode As String = "") As Double
        ...
        If 1 = 1 Then
            ...
            Else
                oRateRS.DoQuery("Select sum(isnull(U_Z_DEDUC_VALUE,0)) from [@Z_PAY2] where U_Z_EMPID='" & aCode & "' and U_Z_DEDUC_TYPE in (Select U_Z_CODE from [@Z_PAY_OLEMAP] where isnull(U_Z_Type,'E')='D' and  isnull(U_Z_EFFPAY,'N')='Y' and U_Z_LEVCODE='" & LeaveCode & "')")
            End If
            dblBasic = dblBasic
            ...
            If strECode <> "" And strType <> "" Then
                ...
                If strMonth <> "" And strYear <> "" Then
                    ...
                    If strStartDate = "" Then
                        ...
                        Return False
                    Else
                        dtDate = aGrid.DataTable.GetValue("U_Z_StartDate", intRow)
                    End If
                    ...
                    If strType = "E" And aGrid.DataTable.GetValue("U_Z_AffDedu", intRow) = "Y" Then
                        oComboColumn = aGrid.Columns.Item("U_Z_DedMonth")
                        If Trim(aGrid.DataTable.GetValue("U_Z_DedMonth", intRow)) = "" Or aGrid.DataTable.GetValue("U_Z_DedMonth", intRow) = "0" Then
                            aGrid.DataTable.SetValue("U_Z_DedMonth", intRow, Month(dtDate).ToString)
                        End If
                        oComboColumn = aGrid.Columns.Item("U_Z_DedYear")
                        If aGrid.DataTable.GetValue("U_Z_DedYear", intRow) = "" Then
                            aGrid.DataTable.SetValue("U_Z_DedYear", intRow, Year(dtDate).ToString)
                        End If
                        Dim stDate As String = aGrid.DataTable.GetValue("U_Z_DedYear", intRow) & CInt(aGrid.DataTable.GetValue("U_Z_DedMonth", intRow)).ToString("00") & "01"
                        Dim stDate1 As String = Year(dtDate) & Month(dtDate).ToString("00") & "01"
                        Dim dtdate1 As Date = oApplication.Utilities.GetDateTimeValue(stDate)
                        Dim dtdate2 As Date = oApplication.Utilities.GetDateTimeValue(stDate1)
                        If dtdate1 < dtdate2 Then
                            oApplication.Utilities.Message("Deduction Month and year should be greater than Transaction Date", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                            aGrid.Columns.Item("U_Z_DedYear").Click(intRow)
                            Return False
                        End If
                    End If
...
#Region "Item Event"
    Public Overrides Sub ItemEvent(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean)
        Try
            If pVal.FormTypeEx = frm_OffToolTransaction Then
                    ...
                    Case True
                        Select Case pVal.EventType
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_CLICK
                                ...
                                If pVal.ItemUID = "18" Then
                                    If oForm.Items.Item("4").Enabled = False Then
                                        ...
                                        Exit Sub
                                    End If
                                    If pVal.ColUID = "U_Z_DedMonth" Or pVal.ColUID = "U_Z_DedYear" And pVal.Row > 0 Then
                                        oGrid = oForm.Items.Item("18").Specific
                                        If oGrid.DataTable.GetValue("U_Z_AffDedu", pVal.Row) <> "Y" Then
                                            BubbleEvent = False
                                            Exit Sub
                                        End If
                                    End If
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_COMBO_SELECT
                                oForm = oApplication.SBO_Application.Forms.Item(FormUID)
                                If pVal.ItemUID = "18" Then
                                    If pVal.ColUID = "U_Z_DedMonth" Or pVal.ColUID = "U_Z_DedYear" Then
                                        oGrid = oForm.Items.Item("18").Specific
                                        If oGrid.DataTable.GetValue("U_Z_AffDedu", pVal.Row) <> "Y" Then
                                            BubbleEvent = False
                                            Exit Sub
