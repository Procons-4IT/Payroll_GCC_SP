Public Class clsContribution
    ...
    Private Sub LoadForm()
        oForm = oApplication.Utilities.LoadForm(xml_Contribution, frm_Contribution)
...
#Region "FormatGrid"
    Private Sub Formatgrid(ByVal agrid As SAPbouiCOM.Grid)
        ...
        agrid.Columns.Item("Code").TitleObject.Caption = "Contribution type"
        agrid.Columns.Item("Name").TitleObject.Caption = "Contribution Name"
        agrid.Columns.Item("U_Z_FrgnName").TitleObject.Caption = "Second Language Name"
        agrid.Columns.Item("U_Z_CON_GLACC").TitleObject.Caption = "G/L Account"
        oEditTextColumn = agrid.Columns.Item("U_Z_CON_GLACC")
        ...
        agrid.Columns.Item("U_Z_CON_GLACC1").TitleObject.Caption = "Credit G/L Account"
        oEditTextColumn = agrid.Columns.Item("U_Z_CON_GLACC1")
...
#Region "CommitTrans"
    Private Sub Committrans(ByVal strChoice As String)
        ...
        If strChoice = "Cancel" Then
            oTemprec.DoQuery("Update [@Z_PAY_OCON] set Name=Code where Name Like '%_XD'")
        Else
            'oTemprec.DoQuery("Select * from [@Z_PAY_OCON] where Name like '%_XD'")
            'For intRow As Integer = 0 To oTemprec.RecordCount - 1
            '    oItemRec.DoQuery("delete from [@Z_PAY_OCON] where Name='" & oTemprec.Fields.Item("Name").Value & "' and Code='" & oTemprec.Fields.Item("Code").Value & "'")
            '    oTemprec.MoveNext()
            'Next
            oTemprec.DoQuery("Delete from  [@Z_PAY_OCON]  where Name Like '%_XD'")
        End If
        ...
        For intRow As Integer = 0 To oGrid.DataTable.Rows.Count - 1
            ...
            If oGrid.DataTable.GetValue("Code", intRow) <> "" Or oGrid.DataTable.GetValue("Name", intRow) <> "" Then
                ...
                Catch ex As Exception
                    stPosttype = "A"
                End Try
                oUserTable = oApplication.Company.UserTables.Item("Z_PAY_OCON")
...
#End Region
    ...
    Private Function AddToUDT_Employee(ByVal aType As String, ByVal GLAccount As String) As Boolean
        ...
        If strType <> "" Then
            ...
            strQuery = "Update [@Z_PAY3] set U_Z_GLACC='" & GLAccount & "' where U_Z_CONTR_TYPE='" & strType & "'"
        ...
        For intRow As Integer = 0 To oTemp.RecordCount - 1
            If strType <> "" Then
                ...
                If strCode <> "" Then ' oUserTable.GetByKey(strCode) Then
                    ...
                    '' oUserTable.UserFields.Fields.Item("U_Z_CONTR_VALUE").Value = 0
    ...
    Private Sub RemoveRow(ByVal intRow As Integer, ByVal agrid As SAPbouiCOM.Grid)
        ...
        For intRow = 0 To agrid.DataTable.Rows.Count - 1
            If agrid.Rows.IsSelected(intRow) Then
                ...
                '    Exit Sub
                'End If
                If oApplication.Utilities.ValidateDeletionMaster(strCode, "Social Benefit") = False Then
                    Exit Sub
                End If
                oApplication.Utilities.ExecuteSQL(oTemp, "update [@Z_PAY_OCON] set  Name =Name +'_XD'  where Code='" & strCode & "'")
...
#Region "Validate Grid details"
    Private Function validation(ByVal aGrid As SAPbouiCOM.Grid) As Boolean
        ...
        For intRow As Integer = 0 To aGrid.DataTable.Rows.Count - 1
            ...
            If strECode <> "" And strEname <> "" Then
                If oGrid.DataTable.GetValue("U_Z_CON_GLACC", intRow) = "" Then
                    oApplication.Utilities.Message("G/L Account Missing...", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                    oGrid.Columns.Item("U_Z_CON_GLACC").Click(intRow)
                    Return False
                End If
            End If
            ...
            If strECode <> "" And strEname <> "" Then
                If oGrid.DataTable.GetValue("U_Z_CON_GLACC1", intRow) = "" Then
                    oApplication.Utilities.Message("G/L Account Missing...", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                    oGrid.Columns.Item("U_Z_CON_GLACC1").Click(intRow)
                    Return False
                End If
            End If
            ...
            For intInnerLoop As Integer = intRow To aGrid.DataTable.Rows.Count - 1
                ...
                If strECode = strECode1 And intRow <> intInnerLoop Then
                    oApplication.Utilities.Message("Contribution already exists. Code no : " & strECode1, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
...
#Region "Item Event"
    Public Overrides Sub ItemEvent(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean)
        Try
            If pVal.FormTypeEx = frm_Contribution Then
                        ...
                        Select Case pVal.EventType
                            Case SAPbouiCOM.BoEventTypes.et_KEY_DOWN
                                ...
                                If pVal.ItemUID = "5" And pVal.ColUID = "Code" And pVal.CharPressed <> 9 Then
                                    ...
                                    If oApplication.Utilities.ValidateDeletionMaster(oGrid.DataTable.GetValue("Code", pVal.Row), "Social Benefit") = False Then
                                        BubbleEvent = False
                                        Exit Sub
                                    End If
                                End If
