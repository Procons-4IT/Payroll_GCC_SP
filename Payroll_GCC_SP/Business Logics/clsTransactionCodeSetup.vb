Public Class clsTransactionCodeSetup
    Inherits clsBase
    ...
    Private Sub LoadForm()
        If oApplication.Utilities.validateAuthorization(oApplication.Company.UserSignature, frm_TransCode) = False Then
            oApplication.Utilities.Message("You are not authorized to do this action", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
            Exit Sub
        End If
        oForm = oApplication.Utilities.LoadForm(xml_TransCode, frm_TransCode)
...
#Region "Databind"
    Private Sub Databind(ByVal aform As SAPbouiCOM.Form)
        Try
            ...
            dtTemp.ExecuteQuery("Select Code,Name,U_Z_FrgnName,U_Z_TRN_GLACC,U_Z_PostType from [@Z_PAY_OTRNS] order by Code")
...
#Region "FormatGrid"
    Private Sub Formatgrid(ByVal agrid As SAPbouiCOM.Grid)
        ...
        agrid.Columns.Item(0).TitleObject.Caption = "Transaction Code"
        agrid.Columns.Item(1).TitleObject.Caption = "Transaction Name"
        agrid.Columns.Item("U_Z_FrgnName").TitleObject.Caption = "Second Language Name"
        agrid.Columns.Item("U_Z_TRN_GLACC").TitleObject.Caption = "G/L Account"
        oEditTextColumn = agrid.Columns.Item("U_Z_TRN_GLACC")
...
#Region "CommitTrans"
    Private Sub Committrans(ByVal strChoice As String)
        ...
        If strChoice = "Cancel" Then
            oTemprec.DoQuery("Update [@Z_PAY_OTRNS] set Name=Code where Name Like '%D'")
        Else
            oTemprec.DoQuery("Delete from  [@Z_PAY_OTRNS]  where Name Like '%D'")
        End If
        ...
        For intRow As Integer = 0 To oGrid.DataTable.Rows.Count - 1
            ...
            If oGrid.DataTable.GetValue(0, intRow) <> "" Or oGrid.DataTable.GetValue(1, intRow) <> "" Then
                ...
                Catch ex As Exception
                    stPosttype = "A"
                End Try
                oUserTable = oApplication.Company.UserTables.Item("Z_PAY_OTRNS")
                If oUserTable.GetByKey(strECode) Then
                    ...
                    oUserTable.UserFields.Fields.Item("U_Z_TRN_GLACC").Value = oGrid.DataTable.GetValue("U_Z_TRN_GLACC", intRow)
...
#Region "Validate Grid details"
    Private Function validation(ByVal aGrid As SAPbouiCOM.Grid) As Boolean
        ...
        For intRow As Integer = 0 To aGrid.DataTable.Rows.Count - 1
            ...
            If oGrid.DataTable.GetValue("U_Z_TRN_GLACC", intRow) = "" Then
                oApplication.Utilities.Message("G/L Account Missing...", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                oGrid.Columns.Item("U_Z_TRN_GLACC").Click(intRow)
                Return False
            End If
...
#Region "Item Event"
    Public Overrides Sub ItemEvent(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean)
        Try
            If pVal.FormTypeEx = frm_TransCode Then
