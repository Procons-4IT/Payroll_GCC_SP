Public Class clsLeaveEncashement
    Inherits clsBase
    ...
    Private Sub LoadForm()
        ...
        If oApplication.Utilities.validateAuthorization(oApplication.Company.UserSignature, frm_LeaveEncashement) = False Then
            oApplication.Utilities.Message("You are not authorized to do this action", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
            Exit Sub
        End If
        oForm = oApplication.Utilities.LoadForm(xml_LeaveEncashement, frm_LeaveEncashement)
        ...
        Try
            ...
            Select Case aChoice
                Case "Emp"
                    ...
                    oGrid.Columns.Item("U_Z_CashOut").TitleObject.Caption = "Cashout"
                    oGrid.Columns.Item("U_Z_CashOut").Editable = True
                ...
                Case "Trans"
                    ...
                    'oTest.DoQuery("Select Code,Name from [@Z_PAY_LEAVE] where U_Z_PaidLeave<>'U' order by Code")
...
#Region "CommitTrans"
    Private Sub Committrans(ByVal strChoice As String)
        ...
        If strChoice = "Cancel" Then
            oTemprec.DoQuery("Update [@Z_PAY_OLETRANS_OFF] set NAME=CODE where Name Like '%_XD'")
            oTemprec.DoQuery("Update [@Z_PAY_OFFCYCLE] set NAME=CODE where Name Like '%_XD'")
        Else
            oTemprec.DoQuery("Delete from  [@Z_PAY_OLETRANS_OFF]  where NAME Like '%_XD'")
    ...
    Private Function AddtoUDT1(ByVal aform As SAPbouiCOM.Form) As Boolean
        ...
        For intRow As Integer = 0 To oGrid.DataTable.Rows.Count - 1
            ...
            If strType <> "" And oGrid.DataTable.GetValue("U_Z_EMPID", intRow) <> "" Then
                ...
                oUserTable = oApplication.Company.UserTables.Item("Z_PAY_OLETRANS_OFF")
                If oGrid.DataTable.GetValue("Code", intRow) = "" Then
                    strCode = oApplication.Utilities.getMaxCode("@Z_PAY_OLETRANS_OFF", "Code")
                    ...
                    If oGrid.DataTable.GetValue("U_Z_CashOut", intRow) = "" Then
                        oUserTable.UserFields.Fields.Item("U_Z_CashOut").Value = "N"
                    Else
                        oUserTable.UserFields.Fields.Item("U_Z_CashOut").Value = oGrid.DataTable.GetValue("U_Z_CashOut", intRow)
                    End If
                    ...
                    If oUserTable.Add() <> 0 Then
                        ...
                        Return False
                    Else
                        oApplication.Utilities.updateLeaveBalance(oGrid.DataTable.GetValue("U_Z_EMPID", intRow), strType, oGrid.DataTable.GetValue("U_Z_Year", intRow))

                        'oRateRS.DoQuery("Select * from  [@Z_EMP_LEAVE_BALANCE] where U_Z_Year=" & (oGrid.DataTable.GetValue("U_Z_Year", intRow)) & " and U_Z_EmpID='" & oGrid.DataTable.GetValue("U_Z_EMPID", intRow) & "' and U_Z_LeaveCode='" & strType & "'")
                        'Dim strcode1 As String
                        'If oRateRS.RecordCount > 0 Then
                        '    strcode1 = oRateRS.Fields.Item("Code").Value
                        'Else
                        '    strcode1 = "0"
                        'End If
                        'oRateRS.DoQuery("select SUM(U_Z_NoofDays) from [@Z_PAY_OLETRANS_OFF] where U_Z_Posted='Y' and  U_Z_EMPID='" & oGrid.DataTable.GetValue("U_Z_EMPID", intRow) & "' and U_Z_TrnsCode='" & strType & "' and U_Z_Year=" & oGrid.DataTable.GetValue("U_Z_Year", intRow))
                        'Dim dblnoofEncashment As Double = oRateRS.Fields.Item(0).Value
                        'oRateRS.DoQuery("Update [@Z_EMP_LEAVE_BALANCE] set  U_Z_EnCash = " & dblnoofEncashment & " where U_Z_Year=" & (oGrid.DataTable.GetValue("U_Z_Year", intRow)) & " and U_Z_EmpID='" & oGrid.DataTable.GetValue("U_Z_EMPID", intRow) & "' and U_Z_LeaveCode='" & strType & "'")
                        'Dim strquery, stString As String
                        'Dim dblob, dblCarriedforward, dblYearly, dblFinalBalance, dblAccurred, dblTransaction, dblAdjustment As Double

                        'stString = "select isnull(sum(U_Z_NoofDays),0),sum(U_Z_Redim) 'Transaction',sum(U_Z_Adjustment) 'Adjustment',U_Z_EmpID  from [@Z_PAYROLL5] where U_Z_Leavecode='" & strType & "'  and U_Z_Year=" & (oGrid.DataTable.GetValue("U_Z_Year", intRow)) & " and U_Z_EmpID='" & oGrid.DataTable.GetValue("U_Z_EMPID", intRow) & "' group by U_Z_EmpID"
                        'oRateRS.DoQuery(stString)

                        'dblAccurred = oRateRS.Fields.Item(0).Value
                        'dblTransaction = oRateRS.Fields.Item(1).Value
                        'dblAdjustment = oRateRS.Fields.Item(2).Value

                        'strquery = "Select isnull(U_Z_CAFWD,0) 'U_Z_CAFWD',isnull(U_Z_Entile,0) 'Yearly',Code,isnull(U_Z_OB,0)'OB' from [@Z_EMP_LEAVE_BALANCE] where U_Z_EmpID='" & oGrid.DataTable.GetValue("U_Z_EMPID", intRow) & "'  and U_Z_LeaveCode='" & strType & "' and U_Z_Year=" & (oGrid.DataTable.GetValue("U_Z_Year", intRow))
                        'oRateRS.DoQuery(strquery)
                        'dblCarriedforward = oRateRS.Fields.Item("U_Z_CAFWD").Value
                        'dblob = oRateRS.Fields.Item("OB").Value
                        'dblYearly = oRateRS.Fields.Item("Yearly").Value
                        'dblFinalBalance = dblob + dblCarriedforward + dblAccurred - dblTransaction + dblAdjustment + dblnoofEncashment
                        'strquery = "Update [@Z_EMP_LEAVE_BALANCE] set U_Z_Balance='" & dblFinalBalance & "' where code='" & strcode1 & "'" ' U_Z_LeaveCode='" & otemp2.Fields.Item("U_Z_LeaveCode").Value & "' and U_Z_Year=" & ayear
                        'oRateRS.DoQuery(strquery)
...
#End Region
    ...
    Public Sub populateDetails(ByVal agrid As SAPbouiCOM.Grid, ByVal aRow As Integer, ByVal aform As SAPbouiCOM.Form)
        ...
        If oRateRS.RecordCount <= 0 Then
            ...
            '  Exit Sub
        End If
        oRateRS.DoQuery("Select * ,isnull(U_Z_StopProces,'N') 'StopProces' from [@Z_PAY_LEAVE] where Code='" & strLeaveType & "'")
        ...
        If strdate <> "" Then
            ...
            dblAMount = getDailyrate(agrid.DataTable.GetValue("U_Z_EMPID", aRow), "A", dtpostingdate, strLeaveType)
        Else
            dblAMount = getDailyrate(agrid.DataTable.GetValue("U_Z_EMPID", aRow), "A", strLeaveType)
        End If
        ...
        agrid.DataTable.SetValue("U_Z_GLACC", aRow, oRateRS.Fields.Item("U_Z_GLACC").Value)

        agrid.DataTable.SetValue("U_Z_GLACC1", aRow, oRateRS.Fields.Item("U_Z_GLACC1").Value)
        ...
        oApplication.Utilities.updateLeaveBalance(agrid.DataTable.GetValue("U_Z_EMPID", aRow), strLeaveType, dblYear)
        ...
        If strdate1 <> "" Then
            ...
            dtdate2 = agrid.DataTable.GetValue("U_Z_StartDate", aRow)
            If 1 = 2 Then 'aGrid.DataTable.GetValue("U_Z_NoofHours", aRow) <> 0 Then
            ...
            Else
                Dim intDiff As Integer = agrid.DataTable.GetValue("U_Z_NoofDays", aRow) 'DateDiff(DateInterval.Day, dtdate1, dtdate2)
                intDiff = intDiff + 1
                ...
                intDiff = agrid.DataTable.GetValue("U_Z_NoofDays", aRow) ' intDiff - dblHolidays
    ...
    Private Function getDailyrate(ByVal aCode As String, ByVal aLeaveType As String, ByVal dtPayrollDate As Date, Optional ByVal LeaveCode As String = "") As Double
        ...
        If 1 = 1 Then
            ...
            If LeaveCode = "" Then
                s = "Select sum(isnull(U_Z_EARN_VALUE,0)) from [@Z_PAY1] where U_Z_EMPID='" & aCode & "'" & stEarning & " and U_Z_EARN_TYPE in (Select T0.U_Z_CODE from [@Z_PAY_OLEMAP] T0 inner Join [@Z_PAY_LEAVE] T1 on T1.Code=T0.U_Z_Code  where isnull(U_Z_Type,'E')='E' and  isnull(T1.U_Z_PaidLeave,'N')='A' and isnull(T0.U_Z_EFFPAY,'N')='Y' )"
                oRateRS.DoQuery(s)
            Else
                s = "Select sum(isnull(U_Z_EARN_VALUE,0)) from [@Z_PAY1] where U_Z_EMPID='" & aCode & "'" & stEarning & " and U_Z_EARN_TYPE in (Select U_Z_CODE from [@Z_PAY_OLEMAP] where isnull(U_Z_Type,'E')='E' and  isnull(U_Z_EFFPAY,'N')='Y' and U_Z_LEVCODE='" & LeaveCode & "')"
                oRateRS.DoQuery(s)
            End If
            dblBasic = dblBasic
            dblEarning = oRateRS.Fields.Item(0).Value
        ...
        For intRow = 0 To agrid.DataTable.Rows.Count - 1
            If agrid.Rows.IsSelected(intRow) Then
                ...
                If strCode <> "" Then
                    ...
                    End If
                    otemprec = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                    oApplication.Utilities.ExecuteSQL(oTemp, "update [@Z_PAY_OLETRANS_OFF] set  NAME =NAME +'_XD'  where Code='" & strCode & "'")
        ...
        For intRow As Integer = 0 To aGrid.DataTable.Rows.Count - 1
            ...
            If strECode <> "" And strType <> "" Then
                ...
                If strMonth <> "" And strYear <> "" Then
                    ...
                    strEndDate = aGrid.DataTable.GetValue("U_Z_StartDate", intRow)
                    ...
                    ' oComboColumn = aGrid.Columns.Item("U_Z_TrnsCode")
                    oRateRS = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                    strLeaveType = oGrid.DataTable.GetValue("U_Z_TrnsCode", intRow) ' oComboColumn.GetSelectedValue(intRow).Value
                    ...
                    strdate2 = oGrid.DataTable.GetValue("U_Z_StartDate", intRow)
                    If strdate1 <> "" And strdate2 <> "" Then
                        dtdate1 = oGrid.DataTable.GetValue("U_Z_StartDate", intRow)
                        dtdate2 = oGrid.DataTable.GetValue("U_Z_StartDate", intRow)
                        If 1 = 2 Then 'oGrid.DataTable.GetValue("U_Z_NoofHours", intRow) <> 0 Then
                        ...
                        Else
                            ...
                            intDiff = oGrid.DataTable.GetValue("U_Z_NoofDays", intRow) ' intDiff + 1
                            Dim dblHolidays As Double = 0 'getHolidayCount(oGrid.DataTable.GetValue("U_Z_EMPID", intRow), oGrid.DataTable.GetValue("U_Z_Cutoff", intRow), dtdate1, dtdate2)
...
#Region "Item Event"
    Public Overrides Sub ItemEvent(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean)
        Try
            If pVal.FormTypeEx = frm_LeaveEncashement Then
                    ...
                    Case True
                        Select Case pVal.EventType
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_CLICK
                                oForm = oApplication.SBO_Application.Forms.Item(FormUID)
                                If pVal.ItemUID = "18" And pVal.Row > 0 Then
                                    oGrid = oForm.Items.Item("18").Specific
                                    If oGrid.DataTable.GetValue("U_Z_Posted", pVal.Row) = "Y" Then
                                        BubbleEvent = False
                                        Exit Sub
                                    End If
                                End If
                                If (pVal.ItemUID = "18" And (pVal.ColUID = "U_Z_IsTerm" Or pVal.ColUID = "U_Z_RejoinDate")) And pVal.Row > 0 Then
                                    oGrid = oForm.Items.Item("18").Specific
                                    oCheck = oGrid.Columns.Item("U_Z_OffCycle")
                                    If oCheck.IsChecked(pVal.Row) = False Then
                                        BubbleEvent = False
                        ...
                        Select Case pVal.EventType
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_KEY_DOWN
                                ...
                                If pVal.ItemUID = "18" And (pVal.ColUID = "U_Z_StartDate" Or pVal.ColUID = "U_Z_EndDate") And pVal.CharPressed = 9 Then
                                    ...
                                    strdate2 = oGrid.DataTable.GetValue("U_Z_StartDate", pVal.Row)
                                    ...
                                    If strdate1 <> "" And strdate2 <> "" Then
                                        Try
                                            ...
                                            dtdate2 = oGrid.DataTable.GetValue("U_Z_StartDate", pVal.Row)
                                            Dim intDiff As Integer = DateDiff(DateInterval.Day, dtdate1, dtdate2)
                                            intDiff = intDiff + 1
                                            ...
                                            intDiff = oGrid.DataTable.GetValue("U_Z_NoofDays", pVal.Row) ' intDiff - dblHolidays
                                            oGrid.DataTable.SetValue("U_Z_NoofDays", pVal.Row, intDiff)
                                            If 1 = 1 Then 'oGrid.DataTable.GetValue("U_Z_NoofHours", pVal.Row) = 0 Then
                                    ...
                                    End If
                                End If
                                If pVal.ItemUID = "18" And pVal.ColUID = "U_Z_NoofDays" And pVal.CharPressed = 9 Then
                                    oGrid = oForm.Items.Item("18").Specific
                                    If oGrid.DataTable.GetValue("U_Z_NoofDays", pVal.Row) > 0 Then
                                        Try
                                            oForm.Freeze(True)
                                            Dim intDiff As Double = getWorkingHours(oGrid.DataTable.GetValue("U_Z_EMPID", pVal.Row))
                                            Dim dblNoofhours As Double = oGrid.DataTable.GetValue("U_Z_NoofDays", pVal.Row)
